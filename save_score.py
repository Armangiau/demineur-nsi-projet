...
def demande_number(message, interval: tuple[int|float,int|float] = (-float('Infinity'), float('Infinity'))) -> int:
...
def get_from_option(message: str, options: set[str]):
    ...
    while user_input not in options:
        print(f'Je ne comprend répondez en choisissant parmis ces options: f{", ".join(options)}')
    ...
    """
        for i, gamer in enumerate(self.hight_score) :
            if self.score_j >= gamer['score'] :
                self.hight_score.insert(i, {"nom" : self.nom_j, "score" : self.score_j})
                return
